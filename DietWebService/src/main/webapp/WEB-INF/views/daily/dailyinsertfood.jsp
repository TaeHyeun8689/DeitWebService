	...
	<script>
		...
		function updateFoodList(foodlist) {
			...
			for (var i = 0; i < foodlist.length; i++) {
				...
				option.text = "음식 이름 : " + foodlist[i].food_name + " 음식 그람 : "
						+ foodlist[i].gram + " 음식 칼로리 : " + foodlist[i].kcal;
		...
		selectElement.addEventListener('change', function() {
			...
			resultItem.textContent = "추가된 음식: " + selectedFood.food_name
					+ " 그람: " + selectedFood.gram + " 칼로리: "
					+ selectedFood.kcal;
			foodResultsDiv.appendChild(resultItem);
		});
		function updateSelectedValue() {
			var selectElement = document.getElementById("cuisine");
